...
def simulations((sim, nodes, W, tf)):
  ...
  if len(ts) > 0:
    ...
    logger.info("# Storing Sim ({})".format(sim))
  
    df = pd.DataFrame(np.vstack(ts))
    df.columns = ['simulations','node_infected','infected_by','t_node_infected', 't_infected_by']
    
    df.to_csv('simulations/simulations_discrete_' + str(sim) + '_' + date_time + '.csv', index=False)
...
def main(n=10):
  ...
  pool.map(simulations, [(sim, nodes, W, tf) for sim in range(1, n_simulations + 1)])
...
if __name__ == '__main__':
    ...
    # Run script: nohup python sir_discrete_simulations_parallel.py 50000 > simulations.out 2> simulations.err &
